...
public:
	...
	*/
	void addScaledVector(Vector3D& vector, float scale)
	{
		Quaternion q(0,
			vector.ReturnX() * scale,
			vector.ReturnY() * scale,
			vector.ReturnZ() * scale);
		q *= *this;
		r += q.r * 0.5f;
		i += q.i * 0.5f;
		j += q.j * 0.5f;
	...
	{
		Quaternion q(0, vector.ReturnX(), vector.ReturnY(), vector.ReturnZ());
		(*this) *= q;
	}
};
	...
	const Quaternion &orientation)
{
	...
	transformMatrix.r[3] = XMVectorSetX(transformMatrix.r[3], position.ReturnX());
	transformMatrix.r[3] = XMVectorSetY(transformMatrix.r[3], position.ReturnY());
	transformMatrix.r[3] = XMVectorSetZ(transformMatrix.r[3], position.ReturnZ());
