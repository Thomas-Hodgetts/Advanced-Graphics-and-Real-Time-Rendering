	...
	if(!binaryInFile.good())
	{
		...
		else
		{
			...
			for(unsigned int i = 0; i < numIndices; i++)
			{
				...
				expandedNormals.push_back(XMFLOAT3(1,1,1));
			}
			...
			meshData.vertices = std::vector<Vertex>();
			meshData.indices = std::vector<DWORD>();

			for (unsigned int i = 0; i < numMeshVertices; ++i)
			{
				meshData.vertices.push_back(Vertex(finalVerts[i].Pos.x, finalVerts[i].Pos.y, finalVerts[i].Pos.z, finalVerts[i].TexC.x, finalVerts[i].TexC.y));
				meshData.indices.push_back(meshIndices[i]);
			}
			for (unsigned int i = 0; i < numIndices; ++i)
			{
				meshData.indices.push_back(meshIndices[i]);
